...
use perspt_sdk::{
    check_full_admissibility, classify_failure, AcceptedTrajectory, BarrierEvaluator,
    ...
    NodeTerminalOutcome, ProviderToolCall, RecoveryCascade, ResidualEvent, StaticCatalog,
    ToolCatalog, ToolChoicePolicy, ToolEntry, TurnOutput, VerificationCadence,
};
...
use proposal::{promote_matching_capability, proposal_from, proposal_scope, uncertified_reason};
...
impl ToolLoop<'_> {
    ...
    /// Gate X: the finite-decision bound is enforced, not advisory. Once the
    /// bound is reached the loop refuses to submit — the refusal appends no
    /// gate decision and routes through the ordinary exhaustion terminal.
    fn refuse_past_bound(&self, state: &mut TurnState) -> Result<bool> {
        let decisions_used = state.trajectory.gate_decisions.len() as u64;
        if decisions_used < state.decision_bound {
            return Ok(false);
        }
        emit(
            self.recorder,
    ...
    async fn apply_non_mutating(
        ...
        if call.name == "read_artifact" {
            return read_artifact_window(self.recorder, &call.arguments);
        }
...
/// entry from the measured baseline; the loop refuses the `(N_gate + 1)`-th
/// gate submission and terminates through the exhaustion path (Gate X).
