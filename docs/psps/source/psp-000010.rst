...
.. math::
   ...
   R(r, s) = \begin{cases}
     ...
     t_{\mathrm{fb},s} & \text{if } s \text{ is a domain stage with a declared
       fallback,}\\
     t_{\mathrm{neu},s} & \text{if } s \text{ is a platform stage,}\\
     \bot        & \text{otherwise.}
   \end{cases}

The cases apply in order; the first matching case selects the template, so
:math:`R` is a function. :math:`R(r,s) = \bot` fails closed. Domain stages
may declare a fallback. Every platform stage resolves to the **neutral
platform route** :math:`t_{\mathrm{neu},s}` when no family template exists.
A model family outside the supported set, including
``ModelFamily::Other``, therefore receives the neutral template, never a
route error. The platform manifest must supply a neutral template for every
platform stage; an incomplete neutral set fails manifest validation.
Compilation is deterministic for a fixed template,
...
.. code-block:: rust
   ...
   pub enum PromptStage {
       // platform stages (neutral route is the guaranteed backstop)
...
.. graphviz::
   :caption: Prompt resolution is deterministic. Platform stages resolve to
             the neutral route; an undeclared domain fallback fails closed.
   :align: center
   ...
   digraph route_resolution {
       ...
       neutral [label="Neutral platform template", fillcolor="#FFF8E6"];
       ...
       family -> fallback [label="no, domain stage"];
       family -> neutral [label="no, platform stage"];
       neutral -> compile [label="manifest-complete"];
       fallback -> compile [label="yes"];
       fallback -> fail [label="no"];
   }
...
.. list-table::
   ...
   * - Y
     - **Prompt route determinism.** Resolution follows exact model, then
       family, then declared domain fallback or the neutral platform route.
       The platform manifest supplies a neutral template for every platform
       stage. Identical inputs yield identical rendered bytes and digests.
     - Two compilations of one ``(template, version, variables)`` differing
       in ``rendered_hash``, a platform manifest validating with an
       incomplete neutral set, or a domain fallback used without a manifest
       declaration.
   * - Z
...
* **Byte-identical template migration first.** The five inline prompts move
  into the neutral platform route verbatim. Supported family directories
  start as copies of it. This gives no immediate quality gain. It gives later prompt changes
  a known baseline.
...
.. list-table::
   ...
   * - 5 - Platform prompt library
     ...
     - ``crates/perspt-core/prompts/``; move all five inline prompts
       byte-identically into ``neutral/``, with supported family
       directories starting as copies; remove the worker's
...
domain routes use the declared fallback; a family outside the supported
set, including ``ModelFamily::Other``, resolves every platform stage to the
neutral route; a platform manifest with an incomplete neutral set fails
validation; unknown/missing variables fail compilation; rendered output and
...
6. **Migration precedes measured divergence.** The current prompts move
   verbatim into the neutral platform route; supported family directories
   start as copies of it. Every later change is a reviewed snapshot diff and
   remains experimental until Gate AE passes. Domain stages may use a
   declared fallback; platform stages resolve to the neutral route when no
   family template exists, so an unsupported model family never fails route
   activation.
