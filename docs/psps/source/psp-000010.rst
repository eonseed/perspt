...
.. list-table::
   ...
   * - 1 - Enforcement debts
     ...
     - Decision bound enforced at the gate and loop layers; trajectory
       ...
       stages wired; architect goes through the admissibility kernel.
   * - 2 - Candidate identity
     - W
     - Implemented
   ...
   * - 3 - SDK prompt sections and codegen
     ...
     - ``perspt_sdk::prompt`` (sections, dialects, budget fitting,
       ...
       codegen crate (build.rs + ``include!(OUT_DIR)``).
   * - 4 - Route and ToolEntry
     - Y
     - Implemented
     - ``adapter_kind``/``prompt_route`` on ``ModelTransport``;
       ``ToolDescriptionLibrary`` with byte-identical defaults.
   * - 5 - Platform section library
     - Y, Z, AE
     - Implemented
     - Five literals migrated byte-identically (``prompt_matrix.rs``
       golden asserts); composed programs recorded per call; Gate AE
       activation surface in the manifest types.
   * - 6 - Domain prompts and verifiers
     ...
     - Structured diagnostics (cargo JSON, ty, Pyright, pytest JUnit,
       ...
       through the coding ``branch_correct`` stage; ty primary with
       fingerprinted Pyright fallback.
   ...
   * - 8 - Adaptive branch execution
     ...
     - Preview/commit forest on the Refine rung (``initial_branches = 1``
       ...
       exact no-goods; quantum granularity v1 is one full branch attempt
       and forest resume is a deterministic re-run from the node
       durable checkpoint.
   * - 9 - ActorTurnRunner
     - Z, AD, AF
     - Implemented
     - Six actors (worker, explorer, architect, adjudicator, summarizer,
       capability probe) on one runner with shared failover and the
       resident-context feasibility gate. The TUI chat loop
       (``external_tools/chat.rs``) is out of scope; the live
       ``context_recall`` tool is deferred until handler services carry
       store access.
   * - 10 - Graph staging
     - AA
     - Implemented
   ...
   * - 11 - CLI, bundles, and evaluation
     ...
     - ``perspt prompts`` list/render/lint/manifest/explain-session and
       ...
       conformance test; seven-arm ladder in ``examples/psp10_eval.rs``
       (live, credential-gated, never CI). Live bundle substitution into
       composed programs waits for override resolution; the adaptive
       default stays ``initial_branches = 1`` until the recorded
       evaluation meets the acceptance rule — the flip is a separate
       reviewed commit.
