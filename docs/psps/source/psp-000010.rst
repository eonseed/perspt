...
.. list-table::
   ...
   * - 11 - CLI, bundles, and evaluation
     ...
     - ``perspt prompts`` list/render/lint/manifest/explain-session and
       ...
       ``examples/eval_tasks/`` corpus with **withheld** oracle overlays
       (erratum 10), per-cell process metrics plus search/paging
       counters, and a ``gate_ae_ready`` flag that stays false below 30
       paired tasks. Live bundle substitution is wired: a validated
...
18. **Checkpoints carry page provenance.** The durable candidate
    ...
    the remaining pages carry the restored root (their true birth root is
    unrecorded — a documented residual for old ledgers only).
