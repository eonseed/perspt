...
.. list-table::
   ...
   * - 9 - ActorTurnRunner
     ...
     - Six actors (worker, explorer, architect, adjudicator, summarizer,
       ...
       infeasible mandatory closure makes no transport call. The TUI
       chat loop (``external_tools/chat.rs``) is out of scope; the live
   ...
   * - 11 - CLI, bundles, and evaluation
     ...
     - ``perspt prompts`` list/render/lint/manifest/explain-session and
       ...
       metrics. Live bundle substitution into composed programs waits
       for override resolution; the adaptive default stays
       ``initial_branches = 1`` until the recorded evaluation meets the
       acceptance rule — the flip is a separate reviewed commit.
