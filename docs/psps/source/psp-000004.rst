...
The multi-agent workflow follows a deterministic state machine to ensure predictable behavior:

.. code-block:: text

    +-------------+     +------------+     +-------------+     +-----------+
    | TASK_QUEUED | --> | PLANNING   | --> | CODING      | --> | REVIEWING |
    +-------------+     +------------+     +-------------+     +-----------+
          |                   |                  |                   |
          v                   v                  v                   v
    [User Cancels]      [Plan Rejected]    [Compile Fail]     [Review Fail]
          |                   |                  |                   |
          v                   v                  v                   v
    +-------------+     +------------+     +-------------+     +-----------+
    |   ABORTED   |     |   FAILED   |     | RETRY(<=3)  |     | RETRY     |
    +-------------+     +------------+     +-------------+     +-----------+
                                                 |                   |
                                                 v                   v
...
*   ``PLANNING`` -> ``CODING`` (on plan approval) or ``FAILED`` (on rejection)
*   ``CODING`` -> ``REVIEWING`` (on successful compile) or ``RETRY`` (on compile fail)
*   ``REVIEWING`` -> ``COMPLETED`` (on approval) or ``RETRY`` (on rejection)
*   ``RETRY`` -> ``CODING``/``REVIEWING`` (retry attempt) or ``FAILED``/``ESCALATED`` (budget exhausted)
