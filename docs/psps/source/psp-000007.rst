...
.. note:: Superseded by PSP-8 — the energy is now quadratic.

   This PSP still uses the **linear** notation, but as of **PSP-8 (System 2)**
   the canonical, implemented energy is the **quadratic residual energy**
   :math:`V(x) = \sum_{e \in E} w_e \lVert r_e(x) \rVert^2`. The components are now
   *derived rollups* of this single energy (no separate
   :math:`\alpha/\beta/\gamma` pass), and ``--energy-weights`` scales the
   per-component weights proportionally. The :math:`V_{boot}` activation goal
   below is realised by emitting typed ``SensorUnavailable``/``ToolFailure``
   residuals into the boot component. See :doc:`psp-000008`.
