...
*   **Simple Prompt:** A simple, Unix-like prompt (for example, ``>`` followed by a space) will be displayed for user input.
...
*   **Prompt:** A simple, non-intrusive prompt (for example, ``>`` followed by a space).
