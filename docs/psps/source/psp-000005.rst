...
* Test coverage: 123 tests across the workspace covering normalization, plugin capabilities, verifier runners, degraded-state propagation, policy rejection, and orchestrator routing.
...
* Test coverage: 140 tests across the workspace covering Phase 5 type construction, serialization, graph rewrite correctness, validator targeting, and classification defaults.
...
* Test coverage: 224 tests across the workspace, including 16 new tests covering workspace classification (empty dir, existing Rust/Python/JS, ambiguous, greenfield with language inference), tool prerequisite checking, ``required_binaries()`` declarations, and model fallback configuration.
...
Closes four bugs identified during live testing where generated projects compiled speculatively but were never verified by the host toolchain:

1. **Bundle command execution** — Artifact bundle commands (e.g. ``cargo add``, ``pip install``) parsed by step_speculate are now executed via the approval-gated tool system instead of being logged as "not yet wired".
2. **Per-node-class verification stages** — ``verification_stages_for_node()`` maps each ``NodeClass`` to the appropriate verification stages: Interface → SyntaxCheck; Implementation → SyntaxCheck + Build (+ Test when weighted_tests present); Integration → SyntaxCheck + Build + Test + Lint. Replaces the old filter that only ran tests for Integration nodes.
3. **Short-circuit verification** — ``run_plugin_verification()`` accepts an ``allowed_stages`` filter and a ``working_dir`` parameter. Syntax failure skips build/test/lint; build failure skips test/lint. Each stage maps to energy components: syntax fail → V_syn ≤ 5.0, build fail → V_syn ≤ 8.0, test fail → V_log, lint fail → V_str += 0.3.
4. **Artifact bundle persistence** — ``ledger.record_artifact_bundle()`` is now called in both the commit path and the escalation path, ensuring DuckDB records which commands were applied for each node.
5. **Correction-loop feedback** — Verification raw output is fed into ``context.last_test_output`` so subsequent correction prompts include actual compiler/test errors. The correction prompt now includes raw ``cargo check``/``cargo build`` stderr alongside LSP diagnostics.
6. **Actuator prompt guidance** — Instruction added for intra-crate imports (``crate::`` in library modules vs. package name in tests/examples/main). Instruction #13 added requiring the LLM to populate the ``commands`` array with dependency install commands (``cargo add``, ``pip install``, ``npm install``) when code uses external crates/packages not already in the project manifest.
7. **Auto-dependency repair** — After ``run_plugin_verification()`` detects a syntax/build failure, ``extract_missing_crates()`` parses compiler stderr for patterns such as "undeclared crate or module", "can't find crate for", and "unresolved import". When missing crates are found, ``auto_install_crate_deps()`` runs ``cargo add <crate>`` automatically and re-runs verification. This closed the gap where generated projects would fail to compile due to missing dependencies in ``Cargo.toml``.
8. **Correction command extraction** — The correction loop now parses ``Commands:`` sections from LLM correction responses and executes whitelisted dependency commands (``cargo add``, ``pip install``, ``npm install``, ``yarn add``, ``pnpm add``) before re-verification. The correction prompt output format was extended with a ``Commands:`` block.
...
* ``crates/perspt-agent/src/agent.rs``: added ``crate::`` import guidance and dependency command instructions to actuator coding prompt, updated multi-artifact bundle JSON example to show populated ``commands`` array.

**Phase 10: uv-first Python developer experience** *(implemented)*

Addresses the quality gap in Python project generation identified during live testing: all nodes in a Python ETL pipeline escalated despite generating synactically valid files, because missing third-party dependencies were never installed, package layout was inconsistent (top-level vs. src-layout), and test files referenced symbols that didn't exist in the generated code.

1. **Python auto-dependency repair** — ``extract_missing_python_modules()`` parses ``ModuleNotFoundError`` and ``ImportError`` patterns from verification output, including nested ``No module named 'foo.bar'`` variants. ``python_import_to_package()`` maps common import-name mismatches (e.g. ``PIL`` → ``pillow``, ``yaml`` → ``pyyaml``, ``cv2`` → ``opencv-python``, ``sklearn`` → ``scikit-learn``). ``auto_install_python_deps()`` runs ``uv add <package>`` for each missing module and follows with ``uv sync --dev`` to refresh the venv. On success, verification is re-run automatically.
2. **Bundle command normalization** — Before executing bundle commands for Python nodes, the orchestrator normalizes generic pip/pip3 install commands to their uv equivalents: ``pip install foo`` → ``uv add foo``, ``python -m pip install foo`` → ``uv add foo``, ``pip install -r requirements.txt`` → ``uv pip install -r requirements.txt``. Non-Python commands and already-correct uv commands pass through unchanged.
3. **Post-bundle uv sync** — After executing bundle commands for Python nodes, ``uv sync --dev`` is run automatically to ensure newly-added dependencies are available in the venv before verification begins.
4. **Python-specific actuator prompt guidance** — Instruction #14 in the actuator prompt now directs the LLM to: prefer src-layout (``src/<package>/``), keep all modules inside the declared package, use relative imports within the package, put tests in ``tests/`` using real symbol names from generated code, and emit ``uv add`` (not ``pip install``) for dependency commands.
5. **Correction prompt improvements** — The ``Commands:`` output format now includes ``uv add`` examples. Fix direction for import errors now recommends ``uv add <pkg>`` and relative imports. The correction command extractor allowlist was extended with ``uv add`` and ``uv pip install``.
6. **Improved Python environment setup** — ``PythonTestRunner::setup_environment()`` now ensures pytest is available as a dev dependency after ``uv sync --dev``: if pytest is not found, it runs ``uv add --dev pytest`` before falling back to ``uv pip install pytest``.
7. **Dynamic run command** — ``PythonPlugin::run_command_for_dir()`` inspects the project directory to return context-appropriate run commands instead of the hardcoded ``uv run python -m main``. It checks ``src/<pkg>/`` directories first, then ``[project.scripts]`` entries in ``pyproject.toml``, defaulting to the generic command only when neither is found. The ``LanguagePlugin`` trait now includes ``run_command_for_dir()`` with a default implementation delegating to ``run_command()``.

Files changed:

* ``crates/perspt-agent/src/orchestrator.rs``: ``extract_missing_python_modules()`` with stdlib filter, ``python_import_to_package()`` mapping table, ``auto_install_python_deps()`` with uv add + uv sync, ``normalize_command_to_uv()`` command rewriter, Python auto-repair wiring in ``step_verify``, bundle command normalization in ``step_speculate``, post-bundle ``uv sync --dev``, extended command allowlist with ``uv add`` and ``uv pip install``. Nine new unit tests for module extraction, package mapping, command normalization, and correction parsing.
...
* ``crates/perspt-core/src/plugin.rs``: ``run_command_for_dir()`` added to ``LanguagePlugin`` trait with default implementation, ``PythonPlugin::run_command_for_dir()`` inspects src layout and ``[project.scripts]``. Three new unit tests for run command detection.
