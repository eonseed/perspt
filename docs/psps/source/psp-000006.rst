...
.. rubric:: Page 3: Energy Convergence
...
where :math:`V(x)` is the composite verification energy for a node and stability is reached when :math:`V(x) \leq \varepsilon`.

.. note:: Superseded by PSP-8 — the energy is now quadratic.

   The dashboard now plots the **quadratic residual energy**
   :math:`V(x) = \sum_{e \in E} w_e \lVert r_e(x) \rVert^2` (PSP-8 System 2). The
   per-component lines :math:`V_{syn}, V_{str}, V_{log}, V_{boot}, V_{sheaf}` are
   *derived rollups* of this single energy rather than independently weighted
   terms; ``--energy-weights`` scales the per-component weights proportionally.
   See :doc:`psp-000008`.
...
* **Energy Chart:** An inline SVG chart (rendered server-side by Askama as ``<svg>`` elements -- no JS charting library needed) showing energy over attempt iterations:
   ...
   - Six lines, each color-coded:
      - :math:`V_{syn}` (syntax) -- ``stroke: oklch(var(--p))`` (primary)
      - :math:`V_{str}` (structural) -- ``stroke: oklch(var(--s))`` (secondary)
      - :math:`V_{log}` (logic/tests) -- ``stroke: oklch(var(--a))`` (accent)
      - :math:`V_{boot}` (bootstrap) -- ``stroke: oklch(var(--in))`` (info)
      - :math:`V_{sheaf}` (sheaf) -- ``stroke: oklch(var(--wa))`` (warning)
      - :math:`V(x)` (composite) -- ``stroke: oklch(var(--er))`` (error), thicker line
   - Horizontal dashed line at :math:`\varepsilon = 0.10` (stability threshold) labeled "Stable below"
   - Filled area below :math:`\varepsilon` in subtle green to visually indicate the stable region
...
* **Convergence Indicator:** A Daisy UI ``alert`` at the top:
   ...
   - ``alert-success`` if :math:`V(x)` is monotonically decreasing over the last 3 attempts
   - ``alert-warning`` if :math:`V(x)` is oscillating
   - ``alert-error`` if :math:`V(x)` is increasing and approaching escalation
...
.. rubric:: Verification Results Grid
...
.. rubric:: Context Provenance Audit
...
.. rubric:: Real-Time Update Architecture (Axum SSE + HTMX)
