...
- **Live Proposal Ensembles (Gate M/T)** - After a gate failure the refine rung can draw a distinct-family proposer round (``[ensemble]`` / ``--ensemble-width``; default off): each candidate is one ledgered gate decision in its own reversible overlay, selection is strictly by measured energy, and ``parity_bench`` gained the heterogeneous-pair-vs-same-model arm so the policy can be rejected by measurement.
