...
- **Latest MCP Client Wired Into Runtime and Chat (Gates K/L/U)** - The custom compatibility transport was replaced by the official Rust SDK and a strict MCP 2026-07-28 ``server/discover`` lifecycle with no legacy fallback. Stdio and stateless Streamable HTTP now cover tools, resources/templates, prompts/completions, explicit roots, opt-in sampling/elicitation, typed notifications, ``subscriptions/listen``, MRTR, and bounded tasks. Remote tools remain locally admitted, bracketed, and replayed; protocol context primitives use reserved read-only operations.
- **MCP TUI and Boundary Hardening** - ``/mcp`` and ``/help`` expose status, policy rejection, and elicitation actions; chat accepts interactive form/URL requests while the non-interactive agent declines them. Sampling preserves the local model route and forwards generation controls, subscription changes re-admit catalogs, task and sampling budgets are bounded, stdio children inherit no ambient credentials beyond executable-resolution variables, and bracketed paste now inserts Unicode/multiline clipboard content into the chat input.
...
- **Promotion TOCTOU Hardening** - Promotion, resume recovery, and rollback share one engine. Unix uses ``openat``/``renameat``/``unlinkat`` with ``O_NOFOLLOW`` component walks and held directory descriptors. Native Windows uses write-through replacement and rejects observed reparse points in its explicitly weaker reduced-isolation mode. Rollback failures are reported, never swallowed. The scheduler now tracks refined generations exactly.
...
- **Evolving Test Evidence** - Production coding gates now default to ``[verification] test_policy = "evolving"``: the resulting implementation, tests, and project configuration are measured together, so an intentional contract change is not forced to satisfy a superseded test. ``backward-compatible`` explicitly adds the restored historical regression view, while ``external-oracle`` overlays separately protected acceptance material on a private candidate copy and adds its command as a blocking test-stage verdict. The selected policy is shared by node, search-preview, and integration measurements and recorded at session start; benchmark hidden oracles remain withheld and fixed.
...
- **Shared MCP Runtime Foundation** - Introduced the governed external-runtime boundary, mode filtering, local effect/footprint admission, namespacing, bounded observations, environment-referenced secrets, replay-only observations, and explicit reconciliation. The current release supersedes its initial tool-only transport with the official latest SDK client described above.
...
- **Native Windows Capability Contract** - TUI chat, simple-chat, MCP, and the
  coding workflow compile natively. Coding process execution stays fail-closed
  by default and requires ``--allow-unisolated`` or
  ``[verification] allow_unisolated = true`` until a restricted-token/ACL
  sandbox backend exists; CI tests strict refusal and the acknowledged native
  verifier path separately.
